...
namespace Domain.Tests
{
    ...
    public class ProductServiceTests
    {
        ...
        //тест был удалЄн так как повтор€лс€
        // тесты отсюда были убраны так как они повтор€ли тесты в create receipt и полностью зависело от внутреннего функционала
        ...
        public void CreateReceipt_WithValidItemsAndCustomer_ReturnsReceipt()
        {
            ...
            Assert.Equal(10.99 + (5.99 * 2), result.FinalPrice, 2); // »спользуем точность 2 знака после зап€той
    ...
    //  ласс дл€ построени€ списка ReceiptLine
    ...
    public class Product_Builder
    {
        ...
        // ѕриватный конструктор - создаем через статический метод
        private Product_Builder() { }

        // —татический метод дл€ начала построени€ продукта
        public static Product_Builder ForProduct()
        {
            return new Product_Builder();
        }

        // ћетоды дл€ настройки продукта (возвращают this дл€ fluent interface)
        ...
        // ‘инальный метод построени€ продукта
