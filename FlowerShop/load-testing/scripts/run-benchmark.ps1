param(
  ...
  [string]$OutDir = "load-testing/results"
)

$ErrorActionPreference = "Stop"

$root = Resolve-Path "$PSScriptRoot\..\.."
$composeFile = Join-Path $root "load-testing\docker-compose.yml"
$dockerfile = Join-Path $root "load-testing\Dockerfile.webapp2"
$k6Dir = Join-Path $root "load-testing\k6"
...
function Start-StatsCollector {
  ...
  $collector = Join-Path $root "load-testing\tools\collect-docker-stats.ps1"
