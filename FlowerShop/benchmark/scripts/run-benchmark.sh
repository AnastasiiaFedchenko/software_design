...
COMPOSE_FILE="$ROOT_DIR/benchmark/docker-compose.yml"
DOCKERFILE="$ROOT_DIR/benchmark/Dockerfile.webapp2"
...
while (( runs_done < RUNS )); do
  ...
  for scenario in "${SCENARIOS[@]}"; do
    ...
    bash "$ROOT_DIR/benchmark/tools/collect-docker-stats.sh" "$stats_file" "$stop_file" &
    stats_pid=$!
